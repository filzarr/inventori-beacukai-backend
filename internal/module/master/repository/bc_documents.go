...
func (r *masterRepo) GetBcDocuments(ctx context.Context, req *entity.GetBcDocumentsReq) (*entity.GetBcDocumentsResp, error) {
	...
	var (
		...
		args  = make([]any, 0, 3)
		query = `
			SELECT COUNT(*) OVER() AS total_data, id, kategori, no_document, tanggal, kategori_barang
			FROM bc_documents
...
func (r *masterRepo) GetBcDocument(ctx context.Context, req *entity.GetBcDocumentReq) (*entity.GetBcDocumentResp, error) {
	...
	query := `SELECT id, kategori, no_document, kategori_barang, tanggal FROM bc_documents WHERE id = ? AND deleted_at IS NULL`
...
func (r *masterRepo) CreateBcDocument(ctx context.Context, req *entity.CreateBcDocumentReq) (*entity.CreateBcDocumentResp, error) {
	query := `INSERT INTO bc_documents (id, kategori, kategori_barang, no_document, tanggal) VALUES (?, ?, ?, ?, ?)`
	...
	if _, err := r.db.ExecContext(ctx, r.db.Rebind(query), Id, req.Kategori, req.KategoriBarang, req.NoDocument, req.Tanggal); err != nil {
...
func (r *masterRepo) UpdateBcDocument(ctx context.Context, req *entity.UpdateBcDocumentReq) error {
	query := `
		UPDATE bc_documents
		SET kategori = ?, kategori_barang = ?, no_document = ?, tanggal = ?, updated_at = NOW()
		WHERE id = ? AND deleted_at IS NULL`

	if _, err := r.db.ExecContext(ctx, r.db.Rebind(query), req.Kategori, req.KategoriBarang, req.NoDocument, req.Tanggal, req.Id); err != nil {
