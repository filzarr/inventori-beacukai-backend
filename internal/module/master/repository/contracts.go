...
func (r *masterRepo) CreateContract(ctx context.Context, req *entity.CreateContractReq) (*entity.CreateContractResp, error) {
	query := `INSERT INTO contracts (id, no_kontrak, supliers_id, kategori, tanggal) VALUES (?, ?, ?, ?, ?)`
	var resp = new(entity.CreateContractResp)
	id := ulid.Make().String()

	if _, err := r.db.ExecContext(ctx, r.db.Rebind(query), id, req.NoKontrak, req.SupliersId, req.Kategori, req.Tanggal); err != nil {
